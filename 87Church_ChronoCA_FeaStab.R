...
wareTypeData <- merge(wareTypeDataA, ZierdenSGs, by=
                        c("CONTEXT"))

# Now strip out just stable and features

wareTypeDataB <- subset(wareTypeData, ! wareTypeData$FEATURE  %in% NA) 
wareTypeDataC <- subset(wareTypeData, wareTypeData$COMPONENT  == 'STABLE') 

wareTypeData <- rbind(wareTypeDataB, wareTypeDataC)
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  labs( title="87 Church Street - Stable and Herold Features", x="Dimension", y='Proportion of Inertia' ) +
...
#add Zierden SGs
rowScoresCompA <- select(wareTypeData_Unit, unit, ZSG)

rowScoresCompB <- left_join (rowScores, rowScoresCompA, by = 'unit') %>%
  mutate(Component = ifelse(is.na(ZSG),'',ZSG))

##Plot it

library(RColorBrewer)
colorCount = length(unique(rowScoresCompB$ZSG))
...
p5a <- ggplot(dim1ForHist, aes(x = dim1)) +
  ...
  labs(title="87 Church Street - Stable and Herold Features", x="Dimension 1", y="Density") +
...
p5b <- ggplot(dim2ForHist, aes(x = dim2)) +
  ...
  labs(title="87 Church Street - Stable and Herold Features", x="Dimension 2", y="Density") +
  geom_density(fill=NA)
p5b

# Add lines for phase breaks
# Note -- don't do this until after you have removed outliers
p5a1 <- p5a + geom_vline(xintercept=c(-.7, -.1, .5), colour = "gray", linetype = "dashed",
                         size=1)      
p5a1
...
p6 <- ggplot(CA_MCD_Phase,aes(x = Dim1, y = blueMCD, 
  ...
  #geom_text_repel(aes(label= unit), cex=4) +
  theme(plot.title = element_text(hjust = 0.5))+
  labs(title="87 Church Street - Stable and Herold Features", x="Dimension 1", y="BLUE MCD")
p6
